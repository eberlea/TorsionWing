...
%Kinematics of input angles for flapping and rotation 
    ...
    phi = flapamp/180*pi.*(sin(2*pi*fflap*t)+0*sin(2*pi*2*fflap*t)).*(sigd.*(2*pi*fflap*t).^sign)./(sigc+sigd.*(2*pi*fflap*t).^sign);
...
filename = sprintf('Resultsfinal_singlesine_damping%d_periodic%d_flappingamp%d_rotatingamp%d_flapfreq%d_rotfreq%d_samplfreq%d_damping0.15.mat', dampingfactor,periodic,flapamp,rotamp,fflap,frot,sampfreq);
