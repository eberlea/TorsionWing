function extrafigure_explaindifffft(zdisp_fnr, zdisp_fr, zdisp_nfr,t)

fig_pos = [25,25,1000,350];
...
subplot(2,3,2)
plot(f-1.5,2*abs(Y_tot_fnr(1:NFFT/2+1)),'k')
axis([0 100 min max])
%xlabel('f (Hz)','FontName','Times')
set(gca,'XTick',[22 28 50 75])
h=text(-18.24,5.5e-3,'(ai)');
set(h,'FontName','Times')
h = line([22 22],[min max]);
set(h,'LineStyle','--')
set(h,'Color','r')
...
subplot(2,3,5)
plot(f-1.5,2*abs(Y_tot_fr(1:NFFT/2+1)),'k')
axis([0 100 min max])
xlabel('f (Hz)','FontName','Times')
set(gca,'XTick',[22 28 50 75])
h=text(-18.24,5.5e-3,'(bi)');
set(h,'FontName','Times')
h = line([22 22],[min max]);
set(h,'LineStyle','--')
set(h,'Color','r')
